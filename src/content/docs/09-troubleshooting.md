...
| `401 Unauthorized` | Token expired or missing | Access token is expired, malformed, or missing from the request | Request a new access token (see [Step 2](/docs/authentication#step-2-request-an-access-token)). There are no refresh tokens in the client credentials flow; a fresh token request is the retry path |
| `403 Forbidden` | Supplier account inactive | The message reads `Your supplier account is inactive. Operation denied.` Your supplier record has not been activated yet, or was deactivated | Contact [support@liketik.com](mailto:support@liketik.com) to activate your account |
| `403 Forbidden` | Insufficient permissions | Token is valid but lacks the `products.supplier` scope | Request the token with `scope=products.supplier` (see [Step 2](/docs/authentication#step-2-request-an-access-token)). Supplier tokens carry the required role automatically; contact [support@liketik.com](mailto:support@liketik.com) if your account needs reconfiguration |
